...
export default {
  ...
  methods: {
    ...
    //修改购买数量
    handler: throttle(async function (type, count, cart) {
      // console.log(cart.skuId+'   使用'+met+"   变了"+count)
      switch (type) {
        case "add":
          count = 1;
          break;
    ...
    async updateChecked(cart, event) {
      try {
        ...
        await this.$store.dispatch("updateIsChecked", {skuId:cart.skuId,isChecked});
        this.getData();
      } catch (error) {
        alert(error.message);
      }
